...
INSERT INTO ss_id_users (user_uid, name, email, status)
...
SELECT
    (SELECT user_id FROM ss_id_users WHERE email = 'admin@shinespark.com'),
    'local',
...
SELECT
    (SELECT role_id FROM ss_id_roles WHERE name = 'admin'),
    (SELECT permission_id FROM ss_id_permissions WHERE code = 'admin:all')
WHERE NOT EXISTS (
    SELECT 1 FROM ss_id_role_permissions
    WHERE
        role_id = (SELECT role_id FROM ss_id_roles WHERE name = 'admin') AND
        permission_id = (SELECT permission_id FROM ss_id_permissions WHERE code = 'admin:all')
    );
...
SELECT
    ...
    (SELECT role_id FROM ss_id_roles WHERE name = 'admin')
WHERE NOT EXISTS (
    SELECT 1 FROM ss_id_user_roles WHERE user_id = (SELECT user_id FROM ss_id_users WHERE email = 'admin@shinespark.com')
);
